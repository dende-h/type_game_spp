...
const WordToType: React.FC<WordToTypeProps> = ({ word, userInput, jaWord, mode, guideOn }) => {
	...
	const escapeRubyWord: string | undefined = mode === "mania" ? jaWord.replace(/<rt>.*?<\/rt>/g, "") : undefined;
		...
		<VStack mt={8} spacing={0}>
			{mode !== "japanese" && (
				<Text fontSize={{ base: "xl", md: "2xl" }} overflowWrap="break-word">
					{escapeRubyWord ? (
						<span dangerouslySetInnerHTML={{ __html: escapeRubyWord }} />
					) : (
						<span dangerouslySetInnerHTML={{ __html: jaWord }} />
					)}
				</Text>
			)}
